...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-2xl border border-border/60 bg-card/60 px-5 py-4 shadow-[0_8px_30px_rgba(0,0,0,0.15)] backdrop-blur",
        className
      )}
      {...props}
    />
...
const CardContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("flex flex-col gap-4 pt-4", className)} {...props} />
  )
);
