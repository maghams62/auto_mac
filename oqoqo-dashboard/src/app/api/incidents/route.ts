import {
  ...
  listTraceabilityIncidents,
  type TraceabilityIncident,
} from "@/app/api/incidents/traceability-store";
import { collectServiceIds } from "@/lib/incidents/helpers";
...
export async function GET(request: Request) {
  ...
  let traceabilityMeta: Record<string, unknown> = { path: getTraceabilitySourcePath() };
  let payload: TraceabilityIncident[] = [];

  try {
    const result = await listTraceabilityIncidents({
      limit: fetchLimit,
      projectId,
      componentId,
      severity,
    ...
    traceabilityMeta = result.meta;
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load incidents";
    if (message.includes("since must be ISO-8601")) {
      return jsonOk({
        status: "INVALID_REQUEST",
        data: { incidents: [] },
        dependencies: { traceabilityStore: traceabilityMeta },
  ...
  const incidents = payload
    ...
    .filter((incident) => {
      ...
      const services = collectServiceIds(incident);
      return services.includes(serviceId);
    })
    .slice(0, limit);

  if (!incidents.length) {
    console.warn("[incidents] traceability log returned no incidents", {
      path: (traceabilityMeta as any)?.path,
      totalRecords: (traceabilityMeta as any)?.totalRecords,
    });
  }
  ...
  return jsonOk({
    ...
    data: { incidents },
    dependencies: {
      traceabilityStore: traceabilityMeta,
    },
  });
}
...
function mapIncidentRecord(record: TraceabilityIncident): IncidentRecord {
  ...
  return {
    id: record.id ?? `incident-${Date.now()}`,
