...
> 🔎 Tip: Type `/` by itself to open a Raycast-style palette listing every slash command with a short tooltip.
...
**Capabilities:**
- Search only the whitelisted local folders
- Refuse to respond if no relevant files exist
- Summarize retrieved passages without outside knowledge
- Export a concise PDF report and surface it in chat

**Examples:**
```
/report Create a report on Tesla from the local docs
/report Summarize the AI agent architecture files you can access
```

> ⚠️ When no local files match, the agent responds with “I could not find any…” so you can expand the data set instead of receiving hallucinated output.
>
> 💡 Prefer natural language (“Create a report on Tesla”) when you want the
> orchestrator to invoke the same Report Agent as part of a larger workflow.
